...
const getData = async () =>{
  const res = await fetch("../../women-products.json");
  return res.json()
}
export default async function WomenCollection(){
  const enterData = await getData();
  ...
  return (
    ...
    <h1 className="men-heading text-5xl md:text-5xl lg:text-6xl xl:text-6xl 2xl:text-6xl pt-8 flex justify-center">Women's Collection</h1>
    <Separator className="mt-5 bg-black w-full"></Separator>
    <div className="carousel">
    <h1 className="text-6xl -mt-10 flex justify-center">Jeans</h1>
    <Slider {...settings}>
    {enterData.map((enter:any)=>{
  ...
  <div className="p-0">
    <h2 className="text-lg font-medium dark:text-white text-gray-900">{enter.product_name}</h2>
    <div className=" flex items-center">
      <p className="mr-2 text-lg font-semibold text-gray-900 dark:text-white">$20.00</p>
      <p className=" text-base  font-medium text-gray-500 line-through dark:text-gray-300">{enter.product_price}</p>
